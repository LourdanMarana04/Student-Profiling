<x-app-layout>
    <style>
        ...
        .assignment-list { display: grid; gap: 0.75rem; max-height: 280px; overflow: auto; }
    ...
    <div style="background: white; border-radius: 12px; padding: 1.5rem 2rem; margin-bottom: 2rem; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05);">
        <h1 style="font-size: 1.75rem; font-weight: 700; color: #333; margin: 0 0 0.25rem 0;">Add New Faculty Member</h1>
        <p style="font-size: 0.875rem; color: #999; margin: 0;">Create the faculty account and set up their course, section, and student assignments in one place.</p>
    </div>
        ...
        <form method="POST" action="{{ route('faculty.store') }}">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input id="faculty_id" class="form-input @error('faculty_id') border-red-500 @enderror" type="text" name="faculty_id" value="{{ old('faculty_id') }}" required />
                ...
                <div class="form-group">
                    ...
                    <input id="office" class="form-input" type="text" name="office" value="{{ old('office') }}" />
                </div>
                <div class="form-group form-grid full">
                    <div>
                        <label class="form-label" for="specialization">Specialization</label>
                        <textarea id="specialization" name="specialization" class="form-textarea">{{ old('specialization') }}</textarea>
            ...
            <p class="helper-copy">Choose the courses the faculty handles, assign sections, and optionally set the student roster for each selected section. Selecting a section will also link its course to the faculty member.</p>
                    ...
                    <div class="assignment-list">
                        @if($supportsSectionAssignments)
                            @foreach($sections as $section)
                                @php
                                    $selectedStudents = old('section_students.' . $section->id, []);
                                @endphp
                                <div class="assignment-item section-assignment" data-section-id="{{ $section->id }}">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <div class="student-select-block {{ in_array($section->id, old('section_ids', [])) ? 'is-visible' : '' }}">
                                            <label class="form-label" for="section_students_{{ $section->id }}">Assigned Students</label>
                                            <select
                                                id="section_students_{{ $section->id }}"
                                                class="form-select student-select"
                                                name="section_students[{{ $section->id }}][]"
                                                multiple
                                            >
            ...
            <div class="form-actions">
                <a href="{{ route('faculty.index') }}" class="btn-cancel">&larr; Back to Faculty</a>
                <button type="submit" class="btn-submit">Create Faculty</button>
            </div>
        </form>
    </div>
